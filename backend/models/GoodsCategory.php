        ...
        if($html == false){
            ...
            foreach ($categories as $k1=>$category){
                ...
                $html .= '<div class="cat '.($k1==0?'item1':'').'">
                    <h3><a href="'.Url::to(['shop/list']).'?goods_category_id='.$category->id.'">'.$category->name.'</a><b></b></h3>
                    <div class="cat_detail">';
                //遍历该一级分类的二级分类
                $categories2 = $category->children(1)->all();
                foreach ($categories2 as $k2=>$category2){
                    $html .= '<dl '.($k2==0?'class="dl_1st"':'').'>
                            <dt><a href="'.Url::to(['shop/list']).'?goods_category_id='.$category2->id.'">'.$category2->name.'</a></dt>
                            <dd>';
                    //遍历该二级分类的三级分类
                    $categories3 = $category2->children(1)->all();
                    foreach ($categories3 as $category3){
                        $html .= '<a href="'.Url::to(['shop/list']).'?goods_category_id='.$category3->id.'">'.$category3->name.'</a>';
                    }
                    $html .= '</dd>
                        </dl>';
                }
